...
async function createPRForRepo(repoName, branchName, results) {
  ...
  const prTitle = `${ticketId}: ${(results.summary || "").slice(0, 60)} (auto-gen)`;
  ...
  const autoGenFooter = `\n\n---\n> **This was generated using the automatic Linear → Claude Code process for ticket [${ticketId}](${ticketUrl}).**`;

  let prBody;
  if (fs.existsSync("/tmp/pr-body.md")) {
    prBody = fs.readFileSync("/tmp/pr-body.md", "utf8") + autoGenFooter;
